 ...
 */
package pw.bastiaan.github.tests.repo;

import android.test.AndroidTestCase;

import pw.bastiaan.github.ui.repo.RecentRepositories;

import org.eclipse.egit.github.core.User;

/**
 * Unit tests of {@link pw.bastiaan.github.ui.repo.RecentRepositories}
 */
public class RecentRepositoriesTest extends AndroidTestCase {
    ...
    public void testMaxReached() {
        ...
        for (int i = 0; i < RecentRepositories.MAX_SIZE; i++) {
            recent.add(i);
            assertTrue(recent.contains(i));
        }

        recent.add(RecentRepositories.MAX_SIZE + 1);
        assertTrue(recent.contains(RecentRepositories.MAX_SIZE + 1));
        assertFalse(recent.contains(0));

        for (int i = 1; i < RecentRepositories.MAX_SIZE; i++)
            assertTrue(recent.contains(i));
    }
