...
public class LightProgressDialog extends ProgressDialog {
    ...
    public static AlertDialog create(Context context, CharSequence message) {
        if (SDK_INT > FROYO) {
            ...
            dialog.setIndeterminateDrawable(context.getResources().getDrawable(pw.bastiaan.github.R.drawable.spinner));
            return dialog;
        } else {
            AlertDialog dialog = LightAlertDialog.create(context);
            dialog.setInverseBackgroundForced(true);
            View view = LayoutInflater.from(context).inflate(pw.bastiaan.github.R.layout.progress_dialog, null);
            ((TextView) view.findViewById(pw.bastiaan.github.R.id.tv_loading)).setText(message);
            dialog.setView(view);
            return dialog;
        }
    }
